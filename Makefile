...
all:  MEDEMO.PRG  DS-MUSIC.ADPCM

clean:
	rm -f *.prg *.PRG *.asm *.vice-* *.BIN *.PAL *.ADPCM *.zip *.7z *.zip src/melyrics.p8
...
MEDEMO.PRG: src/medemo.p8 src/music.p8 src/melyrics.p8 ME-TITLESCREEN.BIN ME-TITLESCREEN.PAL ME-DEMOSCREEN.BIN ME-DEMOSCREEN.PAL ME-FONT.BIN ME-MUSIC.ADPCM
	p8compile $< -target cx16
	mv medemo.prg MEDEMO.PRG

DSDEMO.PRG: src/dsdemo.p8 src/music.p8 src/dslyrics.p8 DS-TITLESCREEN.BIN DS-TITLESCREEN.PAL DS-DEMOSCREEN.BIN DS-DEMOSCREEN.PAL DS-FONT.BIN DS-MUSIC.ADPCM
	p8compile $< -target cx16
...
src/melyrics.p8: src/convertlyrics.py src/melyrics.txt
	python src/convertlyrics.py src/melyrics.txt > $@

src/dslyrics.p8: src/convertlyrics.py src/dslyrics.txt
	python src/convertlyrics.py src/dslyrics.txt > $@

ME-MUSIC.ADPCM: music/chvrches-warning-call.mp3
	sox $< -c 1 -r 16021 music.temp.wav
	adpcm-xq -y -b8 -4 -r music.temp.wav $@
	python src/musicstats.py music.temp.wav $@
	@rm music.temp.wav
